...
\end{itemize}
\fi
Cette interface n'est certainement pas réaliste, en tout cas pas avec une horloge à haute fréquence dans une technologie moderne.
Il y a une hypothèse de synchronicité des signaux de contrôle et de donnée qu'on ne trouve pas dans les liens série rapide ou les bus d'interface mémoire). Nous l'utilisons essentiellement comme une règle du jeu pour étudier la question de la taille de l'encodage d'un jeu d'instructions.
